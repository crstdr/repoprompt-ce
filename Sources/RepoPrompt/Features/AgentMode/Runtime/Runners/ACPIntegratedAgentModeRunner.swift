...
final class ACPIntegratedAgentModeRunner {
    ...
    private static func explicitSelectedModel(
        ...
        guard agentKind == .openCode || agentKind == .cursor || agentKind == .grokBuild || agentKind == .antigravity else {
            return nil
        }
        ...
        {
            let providerName = agentKind == .antigravity ? "Antigravity" : "Grok Build"
            throw AIProviderError.invalidConfiguration(
                detail: "\(providerName) model `\(model)` is not in the discovered model set. Refresh \(providerName) models and retry."
            )
        }
        return model
    }
