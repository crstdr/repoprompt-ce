    ...
    func execute(args: [String: Value]) async throws -> Value {
        guard let op = AgentMCPToolHelpers.normalizedString(args["op"])?.lowercased() else {
            throw MCPError.invalidParams(
                "agent_session_link op is required. Use list, poll, wait, read, send, or mark_done."
            )
        }
        switch op {
        case "list":
        ...
        default:
            throw MCPError.invalidParams(
                "Unsupported agent_session_link op '\(op)'. Use list, poll, wait, read, send, or mark_done."
            )
        }
    }
